<?php
/**
 * HTML5 Template
 * @version 1.5 stable $Id: category_items_html5.php 0001 2012-09-23 14:00:28Z Rehne $
 * @package Joomla
 * @subpackage FLEXIcontent
 * @copyright (C) 2009 Emmanuel Danan - www.vistamedia.fr
 * @license GNU/GPL v2
 * 
 * FLEXIcontent is a derivative work of the excellent QuickFAQ component
 * @copyright (C) 2008 Christoph Lukes
 * see www.schlu.net for more information
 *
 * FLEXIcontent is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 * 
 * Template webconcept
 * Author: c.oerter
 * Date: Januar 2020	
 * Template: blog_wbc
 * 
 * Alternative Grid Frameworks Bootstrap4 und UIKIT
 * Anpassung fuer Mansory (Mauerwerk) Grid
 * Zusaetzliche Klassen 
 * grid-content 
 * background-secondary
 * background-primary
 */

defined( '_JEXEC' ) or die( 'Restricted access' );
// first define the template name
$blogimages     = new JLayoutFile('templates.blogimages', JPATH_ROOT .'/components/com_flexicontent/layouts');
$displayDataImg = array();

$tmpl = $this->tmpl;
$user = JFactory::getUser();
$lead_use_image                          = $this->params->get('lead_use_image', 1);
$lead_position                           = $this->params->get('lead_position');
$displayDataImg['lead_link_image_to']    = $this->params->get('lead_link_image_to', 0);
$displayDataImg['lead_link_image']       = $this->params->get('lead_link_image', 0);

$intro_use_image                         = $this->params->get('intro_use_image', 1);
$intro_position                          = $this->params->get('intro_position');

$displayDataImg['intro_link_image_to']   = $this->params->get('intro_link_image_to', 0);
$displayDataImg['intro_link_image']      = $this->params->get('lead_link_image', 0);

$img_style_lead       					 = ( $this->params->get('lead_item_image_style') ) ? $this->params->get('lead_item_image_style') : '';
$img_style_intro                         = ( $this->params->get('intro_item_image_style') ) ? $this->params->get('intro_item_image_style') : '';


$lead_link_to_popup                      = $this->params->get('lead_link_to_popup', 0);
$intro_link_to_popup                     = $this->params->get('intro_link_to_popup', 0);

$intro_use_description                   = $this->params->get('intro_use_description', 1);
$lead_use_description                    = $this->params->get('lead_use_description', 1);

$mansoryleadclass      = 'nomansorylead';
$mansoryintroclass     = 'nomansoryintro';

$grid_framework        = $this->params->get('grid_framework', 1);

switch  ( $grid_framework  ) {

		case 0: $displayDataImg['img_style']    = $img_style_lead;   /* Flexicontent */
		        $displayDataImg['img_class']    = '';
				$displayDataImg['img_position'] = '';
				break;
		case 1: $displayDataImg['img_style']     = 'img-'. $img_style_lead;   /* bootstrap 4 */
		        $displayDataImg['img_class']     = 'img-fluid ';
				$displayDataImg['img_position'] = ' float-md-';
				break;			
		case 2: $displayDataImg['img_style']    = 'uk-border-';
				$displayDataImg['img_class']    = 'uk-thumbnail ';
				$displayDataImg['img_position'] = ' uk-align-';   /*  Uikit */
				break;
} 

/*if ($lead_link_to_popup || $intro_link_to_popup) { 
	//flexicontent_html::loadFramework('flexi-lib');
}*/


// Ergaenzung Template Variables webconcept Bootstrap 4 Template




$arr_cols_gridclasses   	=   array(
								array(1=>'col-lg-12',2=>'col-lg-6',3=>'col-lg-4',4=>'col-lg-3'),
           					    array(1=>'uk-width-large-1-1',2=>'uk-width-large-1-2',3=>'uk-width-large-1-3',4=>'uk-width-large-1-4')
           					  );

$arr_cols_gridclasses_sm    =   array(
								array(1=>'col-md-12',2=>'col-md-6',3=>'col-md-4'),
  								array(1=>'uk-width-medium-1-1',2=>'uk-width-medium-1-2',3=>'uk-width-medium-1-3')
  								);
$arr_cols_gridclasses_lg    =   array(
								array(1=>'col-xl-12',2=>'col-xl-6',3=>'col-xl-4'),
  								array(1=>'uk-width-xlarge-1-1',2=>'uk-width-xlarge-1-2',3=>'uk-width-xlarge-1-3')
  								);

$arr_cols_gridclasses_xs   	= array(
								array(1=>'col-12',2=>'col-6',3=>'col-4'),
  								array(1=>'uk-width-small-1-1',2=>'uk-width-small-1-2',3=>'uk-width-small-1-3')
  								);

$arr_cols_gridclasses_dbl  	= array(
								array(1=>'col-lg-12',2=>'col-lg-6',3=>'col-lg-8',4=>'col-lg-6'),
								array(1=>'uk-width-medium-1-1',2=>'uk-width-medium-1-2',3=>'uk-width-medium-2-3',4=>'uk-width-medium-2-4')
								);
$arr_cols_gridclasses_dbl_lg  	= array(
								array(1=>'col-xl-12',2=>'col-xl-6',3=>'col-xl-8',4=>'col-xl-6'),
								array(1=>'uk-width-large-1-1',2=>'uk-width-large-1-2',3=>'uk-width-large-2-3',4=>'uk-width-large-2-4')
								);


	if ( $grid_framework == 1 ) :
			$cols_gridclasses         = $arr_cols_gridclasses[0];
			$cols_gridclasses_sm      = $arr_cols_gridclasses_sm[0];
			$cols_gridclasses_xs      = $arr_cols_gridclasses_xs[0];
			$cols_gridclasses_lg      = $arr_cols_gridclasses_lg[0];
			$cols_gridclasses_dbl     = $arr_cols_gridclasses_dbl[0];
			$cols_gridclasses_dbl_lg  = $arr_cols_gridclasses_dbl_lg[0];

    endif;

	if ( $grid_framework == 2 ) :
			$cols_gridclasses         = $arr_cols_gridclasses[1];
			$cols_gridclasses_sm      = $arr_cols_gridclasses_sm[1];
			$cols_gridclasses_lg      = $arr_cols_gridclasses_lg[1];
			$cols_gridclasses_xs      = $arr_cols_gridclasses_xs[1];
			$cols_gridclasses_dbl     = $arr_cols_gridclasses_dbl[1];
			$cols_gridclasses_dbl_lg  = $arr_cols_gridclasses_dbl_lg[1];
    endif;


// end webconcept 

// MICRODATA 'itemtype' for ALL items in the listing (this is the fallback if the 'itemtype' in content type / item configuration are not set)
$microdata_itemtype_cat = $this->params->get( 'microdata_itemtype_cat', 'Article' );

// ITEMS as MASONRY tiles

if (!empty($this->items) && ($this->params->get('lead_placement', 0)==1 || $this->params->get('intro_placement', 0)==1))
{
	flexicontent_html::loadFramework('masonry');
	flexicontent_html::loadFramework('imagesLoaded');
	
	$js = "
		jQuery(document).ready(function(){
	";



	if ($this->params->get('lead_placement', 0)==1 ) {
		$js .= "
			var container_lead = document.querySelector('.mansorylead');
			var msnry_lead;
			// initialize Masonry after all images have loaded
			if (container_lead) {
				imagesLoaded( container_lead, function() {
					msnry_lead = new Masonry( container_lead, {
						gutter: '.gutter-size',
						columnWidth: '.grid-size',
						itemSelector: '#flexicontent .leadingblock .fc_bloglist_item',
						transitionDuration: '0.8s',
						horizontalOrder: true,
						percentPosition: true

					});
				});
			}
		";
		$mansoryleadclass = '';
	} 


	if ($this->params->get('intro_placement', 0)==1) {
		$js .= "
			var container_intro = document.querySelector('.mansoryintro');
			var msnry_intro;
			// initialize Masonry after all images have loaded
			if (container_intro) {
				imagesLoaded( container_intro, function() {
					msnry_intro = new Masonry( container_intro, {
						gutter: '.gutter-size',
						columnWidth: '.grid-size',
						itemSelector: '#flexicontent .introblock .fc_bloglist_item',
						transitionDuration: '0.8s',
						horizontalOrder: true,
						percentPosition: true						
					});
				});
			}
		";
		$mansoryintroclass = '';
	} 

	$js .= "	
		});
	";
	JFactory::getDocument()->addScriptDeclaration($js);
} 


?>

<?php

ob_start();
include(JPATH_SITE.DS.'templates'.DS.$template.DS.'tmpl_common'.DS.'listings_filter_form_html5.php');
$filter_form_html = trim(ob_get_contents());
ob_end_clean();

if ( $filter_form_html )
{
	echo '
	<div class="fcclear"></div>
	<aside class="group">
		' . $filter_form_html . '
	</aside>';
}

?>


<?php
if (!$this->items) {
	// No items exist
	if ($this->getModel()->getState('limit')) {
		// Not creating a category view without items
		echo '<div class="noitems group">' . JText::_( 'FLEXI_NO_ITEMS_FOUND' ) . '</div>';
	}
	return;
}

$items	= & $this->items;
$count 	= count($items);

// Calculate common data outside the item loops
if ($count) {
	$_read_more_about = JText::_( 'FLEXI_READ_MORE_ABOUT' );
	$displayDataImg['_read_more_about'] = $_read_more_about;
	$displayDataImg['tooltip_class'] = FLEXI_J30GE ? 'hasTooltip' : 'hasTip';
	$_comments_container_params = 'class="fc_comments_count '.$tooltip_class.'" title="'.flexicontent_html::getToolTip('FLEXI_NUM_OF_COMMENTS', 'FLEXI_NUM_OF_COMMENTS_TIP', 1, 1).'"';
}
?>
<div class="content group">

<?php
$leadnum  = $this->params->get('lead_num', 1);
$leadnum  = ($leadnum >= $count) ? $count : $leadnum;

// Handle category block (start of category items)
$doing_cat_order = $this->category->_order_arr[1]=='order';
$lead_catblock  = $this->params->get('lead_catblock', 0);
$intro_catblock = $this->params->get('intro_catblock', 0);
$lead_catblock_title  = $this->params->get('lead_catblock_title', 1);
$intro_catblock_title = $this->params->get('intro_catblock_title', 1);
if ($lead_catblock || $intro_catblock) {
	global $globalcats;
}

// ONLY FIRST PAGE has leading content items
if ($this->limitstart != 0) $leadnum = 0;

$lead_cut_text  = $this->params->get('lead_cut_text', 400);
$intro_cut_text = $this->params->get('intro_cut_text', 200);
$uncut_length = 0;
FlexicontentFields::getFieldDisplay($items, 'text', $values=null, $method='display'); // Render 'text' (description) field for all items

if ($leadnum) :
	//added to intercept more columns (see also css changes)
	$lead_cols    = $this->params->get('lead_cols', 1);
	$lead_cols_sm = $this->params->get('lead_cols_sm', 1);
	$lead_cols_xs = $this->params->get('lead_cols_xs', 1);

	$lead_cols_classes = array(1=>'one',2=>'two',3=>'three',4=>'four');
	$classnum = $lead_cols_classes[$lead_cols];

	// Ergaenzung  webconcept alternative Grid Frameworks
    $mansoryleadstyle         = $this->params->get('lead_mansory_style', 0);
	$lead_placement           = $this->params->get('lead_placement', 0);
	$lead_gridclass_md 		  = $cols_gridclasses[$lead_cols];
	$lead_gridclass_xs        = $cols_gridclasses_xs[$lead_cols_xs];
	$lead_gridclass_sm   	  = $cols_gridclasses_sm[$lead_cols_sm];
	$lead_gridclass_lg   	  = $cols_gridclasses_lg[$lead_cols];
	
?>
<?php if ($this->params->get('lead_placement', 0)==1) : ?>
	<div class="<?php echo ( $grid_framework == 0 ) ? $classnum : ''; ?> <?php echo ($lead_placement == 1) ? 'mansorylead' : '';?>">
		<div class="gutter-size"></div>
		<div class="grid-size <?php echo ( $grid_framework > 0 ) ? $lead_gridclass_xs . ' ' . $lead_gridclass_sm . ' ' . $lead_gridclass_md . ' ' . $lead_gridclass_lg  : ' '; ?>"></div>
<?php endif;

 // end Ergaenzung webconcept ?>

	<div class="leadingblock group <?php echo ( $grid_framework == 0 ) ? $classnum : ''; ?> <?php echo $mansoryleadclass; ?> <?php echo ($grid_framework == 2) ? 'uk-grid' : '';?> <?php echo ( $grid_framework == 1 ) ? 'row' : ''; ?>">
	
		<?php

		if ($lead_use_image) {
			// ergaenzung webconcept
				

			    switch  ( $lead_position ) {
			    		case 0: $displayDataImg['img_position'] .= "left order-0 mr-md-3 mb-3 mt-md-3";
			    				break;
			    		case 1: $displayDataImg['img_position']  .= "right order-1 ml-md-3 mb-3 mt-md-3";
			    				break;			
			    		case 2: $displayDataImg['img_position']  .= "none mt-md-3";
			    				break;
			    }

			  
			if ( $this->params->get('lead_image')) {
				$img_size_map   = array('l'=>'large', 'm'=>'medium', 's'=>'small', 'o'=>'original');
				$img_field_size = $img_size_map[ $this->params->get('lead_image_size' , 'l') ];
				$img_field_name = $this->params->get('lead_image');
			}
		}

		
		
		for ($i=0; $i<$leadnum; $i++) :

			$background = ($i%2) ? 'background-primary' : 'background-secondary';
			$item = $items[$i];
			$fc_item_classes = 'fc_bloglist_item';
			if ($doing_cat_order)
     		$fc_item_classes .= ($i==0 || ($items[$i-1]->rel_catid != $items[$i]->rel_catid) ? ' fc_cat_item_1st' : '');
			// $fc_item_classes .= $i%2 ? ' fceven' : ' fcodd';
			$fc_item_classes .= ' fccol'.($i%$lead_cols + 1);
			$gridclass = '';
			$gridclass_sm = '';
			$gridclass_xs = '';
			// ergaenzung webconcept for Mansory Grid + Bootstrap4 || UIKIT

			if ( $grid_framework > 0 ) : 
					
					// grid 3 phone
					
					$gridclass_xs      .= $cols_gridclasses_xs[$lead_cols_xs] . ' ';
					
					// grid 3 cols tablet
					
					$gridclass_sm      .= $cols_gridclasses_sm[$lead_cols_sm] . ' ';
					
					$gridclass 			= $cols_gridclasses[$lead_cols] . ' ';
					$gridclass 		   .= $cols_gridclasses_lg[$lead_cols];

					/* grid cols desktop*/
					if ( $lead_placement == 1 ) :
												
						switch  ( $mansoryleadstyle ) :
						
							case 0: 
									$gridclass  = $cols_gridclasses[$lead_cols] . ' ';
									$gridclass .= $cols_gridclasses_lg[$lead_cols];
									break;
							case 1: 
									$gridclass = $i%2 ? $cols_gridclasses_dbl[$lead_cols] . ' ' . $cols_gridclasses_dbl_lg[$lead_cols] : $cols_gridclasses[$lead_cols] . ' ' . $cols_gridclasses_lg[$lead_cols];
									break;
							case 2: 
									$gridclass = $i%2 ? $cols_gridclasses[$lead_cols] . ' ' . $cols_gridclasses_lg[$lead_cols] : $cols_gridclasses_dbl[$lead_cols] . ' ' . $cols_gridclasses_dbl_lg[$lead_cols];
									break;
							endswitch;
					endif;	
					$gridclass    .= ' ';
					

				endif; 


			// end webconcept	

			$markup_tags = '<span class="fc_mublock">';
			foreach($item->css_markups as $grp => $css_markups) {
				if ( empty($css_markups) )  continue;
				$fc_item_classes .= ' fc'.implode(' fc', $css_markups);
				
				$ecss_markups  = $item->ecss_markups[$grp];
				$title_markups = $item->title_markups[$grp];
				foreach($css_markups as $mui => $css_markup) {
					$markup_tags .= '<span class="fc_markup mu' . $css_markups[$mui] . $ecss_markups[$mui] .'">' .$title_markups[$mui]. '</span>';
				}
			}
			$markup_tags .= '</span>';
			
			$custom_link = null;
			if ($lead_use_image) :
				if (!empty($img_field_name) && ( array_key_exists($img_field_name, $item->fields)) )
				{
					// Render method 'display_NNNN_src' to avoid CSS/JS being added to the page
					FlexicontentFields::getFieldDisplay($item, $img_field_name, $values=null, $method='display_'.$img_field_size.'_src', 'category');
					
					$img_field = $item->fields[$img_field_name];
					$src = str_replace(JURI::root(), '', @ $img_field->thumbs_src[$img_field_size][0] );
					if ( $displayDataImg['lead_link_image_to'] && isset($img_field->value[0]) ) {
						$custom_link = ($v = unserialize($img_field->value[0])) !== false ? @ $v['link'] : @ $img_field->value[0]['link'];
					}
				} else {
					$src = flexicontent_html::extractimagesrc($item);
				}
				
				// Use default image form layout parameters
				if (!$src && isset($lead_type_default_imgs[$item->typealias]))  $src = $lead_type_default_imgs[$item->typealias];
				if (!$src && isset($lead_type_default_imgs['_OTHER_']))         $src = $lead_type_default_imgs['_OTHER_'];
				
				$RESIZE_FLAG = !$this->params->get('lead_image') || !$this->params->get('lead_image_size');
				if ( $src && $RESIZE_FLAG ) {
					// Resize image when src path is set and RESIZE_FLAG: (a) using image extracted from item main text OR (b) not using image field's already created thumbnails
					$w		= '&amp;w=' . $this->params->get('lead_width', 200);
					$h		= '&amp;h=' . $this->params->get('lead_height', 200);
					$aoe	= '&amp;aoe=1';
					$q		= '&amp;q=95';
					$ar 	= '&amp;ar=x';
					$zc		= $this->params->get('lead_method') ? '&amp;zc=' . $this->params->get('lead_method') : '';
					$ext = strtolower(pathinfo($src, PATHINFO_EXTENSION));
					$f = in_array( $ext, array('png', 'ico', 'gif') ) ? '&amp;f='.$ext : '';
					$conf	= $w . $h . $aoe . $q . $ar . $zc . $f;
					
					$base_url = (!preg_match("#^http|^https|^ftp|^/#i", $src)) ?  JURI::base(true).'/' : '';
					$displayDataImg['thumb'] = JURI::base(true).'/components/com_flexicontent/librairies/phpthumb/phpThumb.php?src='.$base_url.$src.$conf;
				} else {
					// Do not resize image when (a) image src path not set or (b) using image field's already created thumbnails
					$displayDataImg['thumb'] = $src;
				}
			endif;

			$link_url = $custom_link ? $custom_link : JRoute::_(FlexicontentHelperRoute::getItemRoute($item->slug, $item->categoryslug, 0, $item));
			$displayDataImg['link_url'] = $link_url;
			$displayDataImg['title_encoded'] = htmlspecialchars($item->title, ENT_COMPAT, 'UTF-8');
			
			if ($lead_link_to_popup) $_tmpl_ = (strstr($link_url, '?') ? '&' : '?'). 'tmpl=component';


			// MICRODATA document type (itemtype) for each item
			// -- NOTE: category's microdata itemtype is fallback if the microdata itemtype of the CONTENT TYPE / ITEM are not set
			$microdata_itemtype = $item->params->get( 'microdata_itemtype') ? $item->params->get( 'microdata_itemtype') : $microdata_itemtype_cat;
			$microdata_itemtype_code = $microdata_itemtype ? 'itemscope itemtype="http://schema.org/'.$microdata_itemtype.'"' : '';
		?>
		
		<?php echo $lead_catblock ?
			'<div class="lead_catblock">'
				.($lead_catblock_title && @$globalcats[$item->rel_catid] ? $globalcats[$item->rel_catid]->title : '').
			'</div>' : ''; ?>		
		
		<div id="wbc_bloglist_item_<?php echo $i; ?>" class="<?php echo $fc_item_classes; ?> <?php echo ( $grid_framework > 0 ) ? $gridclass_xs . $gridclass_sm . $gridclass  : ''; ?> " <?php echo $microdata_itemtype_code; ?> style="overflow: hidden;">
			
			<?php // if ($this->params->get('show_title', 1)) : ?>
			<article class="group grid-content <?php echo $background;?>">
			<?php // endif; ?>
			
			
			<?php if ($item->event->beforeDisplayContent) : ?>
			<!-- BOF beforeDisplayContent -->
				<aside class="fc_beforeDisplayContent group">
					<?php echo $item->event->beforeDisplayContent; ?>
				</aside>
			<!-- EOF beforeDisplayContent -->
			<?php endif; ?>
			
			<?php
				$header_shown =
					$this->params->get('show_comments_count', 1) ||
					$this->params->get('show_title', 1) || $item->event->afterDisplayTitle ||
					0; // ...
			?>
		
			<?php if ( $header_shown ) : ?>
			<header class="group">
			<?php endif; ?>			
			
			<?php if ($this->params->get('show_editbutton', 1)) : ?>
				<?php $editbutton = flexicontent_html::editbutton( $item, $this->params ); ?>
				<?php $statebutton = flexicontent_html::statebutton( $item, $this->params ); ?>
			<?php endif; ?>			
			<?php $deletebutton = flexicontent_html::deletebutton( $item, $this->params ); ?>
			<?php $approvalbutton = flexicontent_html::approvalbutton( $item, $this->params ); ?>

		
			<?php if ( $editbutton || $statebutton || $deletebutton || $approvalbutton ) : ?>
				<div class="adminbuttons d-flex">
					<?php if ($editbutton) : ?>
							<div class="fc_edit_link"><?php echo $editbutton;?></div>
					<?php endif; ?>

					<?php if ($statebutton) : ?>
							<div class="fc_state_toggle_link"><?php echo $statebutton;?></div>
					<?php endif; ?>
					<?php if ($deletebutton) : ?>
						<div class="fc_delete_link"><?php echo $deletebutton;?></div>
					<?php endif; ?>
					<?php if ($approvalbutton) : ?>
						<div class="fc_approval_request_link"><?php echo $approvalbutton;?></div>
					<?php endif; ?>
				</div>
			<?php endif; ?>

			<?php if ($this->params->get('show_comments_count')) : ?>
				<?php if ( isset($this->comments[ $item->id ]->total) ) : ?>
					<div <?php echo $_comments_container_params; ?> >
						<?php echo $this->comments[ $item->id ]->total; ?>
					</div>
				<?php endif; ?>
			<?php endif; ?>
			
			<?php if ($this->params->get('show_title', 1)) : ?>
				<!-- BOF item title -->
				<h
					<?php if ($this->params->get('link_titles', 0)) : ?>
						<a href="<?php echo $link_url; ?>"><?php echo $item->title; ?></a>
					<?php else : ?>
						<?php echo $item->title; ?>
					<?php endif; ?>
					
				</h3>
				<!-- EOF item title -->
			<?php endif; ?>	
			
			<!-- BOF afterDisplayTitle -->
			<?php if ($item->event->afterDisplayTitle) : ?>
				<div class="fc_afterDisplayTitle group">
					<?php echo $item->event->afterDisplayTitle; ?>
				</div>
			<?php endif; ?>
			<!-- EOF afterDisplayTitle -->

			<?php echo $markup_tags; ?>
			
			<?php if ( $header_shown ) : ?>
			</header>
			<?php endif; ?>
			
			
			<?php if (isset($item->positions['above-description-line1'])) : ?>
			<!-- BOF above-description-line1 block -->
			<div class="lineinfo line1">
				<?php foreach ($item->positions['above-description-line1'] as $field) : ?>
				<div class="element">
					<?php if ($field->label) : ?>
					<span class="flexi label field_<?php echo $field->name; ?>"><?php echo $field->label; ?></span>
					<?php endif; ?>
					<div class="value field_<?php echo $field->name; ?>"><?php echo $field->display; ?></div>
				</div>
				<?php endforeach; ?>
			</div>
			<!-- EOF above-description-line1 block -->
			<?php endif; ?>
			

			
			<?php if (isset($item->positions['above-description-line1-nolabel'])) : ?>
			<!-- BOF above-description-nolabel-line1 block -->
			<div class="lineinfo line1">
				<?php foreach ($item->positions['above-description-line1-nolabel'] as $field) : ?>
				<div class="element">
					<div class="value field_<?php echo $field->name; ?>"><?php echo $field->display; ?></div>
				</div>
				<?php endforeach; ?>
			</div>
			<!-- EOF above-description-nolabel-line1 block -->
			<?php endif; ?>
			
				
			
			<?php if (isset($item->positions['above-description-line2'])) : ?>
			<!-- BOF above-description-line2 block -->
			<div class="lineinfo line2">
				<?php foreach ($item->positions['above-description-line2'] as $field) : ?>
				<div class="element">
					<?php if ($field->label) : ?>
					<span class="flexi label field_<?php echo $field->name; ?>"><?php echo $field->label; ?></span>
					<?php endif; ?>
					<div class="value field_<?php echo $field->name; ?>"><?php echo $field->display; ?></div>
				</div>
				<?php endforeach; ?>
			</div>
			<!-- EOF above-description-line2 block -->
			<?php endif; ?>
			
				
			
			<?php if (isset($item->positions['above-description-line2-nolabel'])) : ?>
			<!-- BOF above-description-nolabel-line2 block -->
			<div class="lineinfo line2">
				<?php foreach ($item->positions['above-description-line2-nolabel'] as $field) : ?>
				<div class="element">
					<div class="value field_<?php echo $field->name; ?>"><?php echo $field->display; ?></div>
				</div>
				<?php endforeach; ?>
			</div>
			<!-- EOF above-description-nolabel-line2 block -->
			<?php endif; ?>
			
				
			<div class="lineinfo image_descr <?php echo $lead_position == 2 ? 'flex-column' : 'flex-row';?> ">
			<?php if ($lead_use_image && $src) : ?>
          
            <?php $displayDataImg['link_image']  = ($this->params->get('lead_link_image', 1)) ? true : false ?>

			<?php echo $blogimages->render($displayDataImg);?>	

			<?php endif; ?>
			  
			<?php if ($lead_use_description) : ?>
			<div class="intro-text mt-md-3">
			<?php
				if ($this->params->get('lead_strip_html', 1)) : ?>
					<p><?php echo flexicontent_html::striptagsandcut( $item->fields['text']->display, $lead_cut_text, $uncut_length );?></p>
				<?php else :
					echo $item->fields['text']->display;
				endif;
			?>
			</div>
			<?php endif; ?>			
			</div>

			
			<?php if (isset($item->positions['under-description-line1'])) : ?>
			<!-- BOF under-description-line1 block -->
			<div class="lineinfo line3">
				<?php foreach ($item->positions['under-description-line1'] as $field) : ?>
				<div class="element">
					<?php if ($field->label) : ?>
					<span class="flexi label field_<?php echo $field->name; ?>"><?php echo $field->label; ?></span>
					<?php endif; ?>
					<div class="value field_<?php echo $field->name; ?>"><?php echo $field->display; ?></div>
				</div>
				<?php endforeach; ?>
			</div>
			<!-- EOF under-description-line1 block -->
			<?php endif; ?>
			
				
			
			<?php if (isset($item->positions['under-description-line1-nolabel'])) : ?>
			<!-- BOF under-description-line1-nolabel block -->
			<div class="lineinfo line3">
				<?php foreach ($item->positions['under-description-line1-nolabel'] as $field) : ?>
				<div class="element">
					<div class="value field_<?php echo $field->name; ?>"><?php echo $field->display; ?></div>
				</div>
				<?php endforeach; ?>
			</div>
			<!-- EOF under-description-line1-nolabel block -->
			<?php endif; ?>
			

			
			<?php if (isset($item->positions['under-description-line2'])) : ?>
			<!-- BOF under-description-line2 block -->
			<div class="lineinfo line4">
				<?php foreach ($item->positions['under-description-line2'] as $field) : ?>
				<div class="element">
					<?php if ($field->label) : ?>
					<span class="flexi label field_<?php echo $field->name; ?>"><?php echo $field->label; ?></span>
					<?php endif; ?>
					<div class="value field_<?php echo $field->name; ?>"><?php echo $field->display; ?></div>
				</div>
				<?php endforeach; ?>
			</div>
			<!-- EOF under-description-line2 block -->
			<?php endif; ?>
			
			
			<?php if (isset($item->positions['under-description-line2-nolabel'])) : ?>
			<!-- BOF under-description-line2-nolabel block -->
			<div class="lineinfo line4">
				<?php foreach ($item->positions['under-description-line2-nolabel'] as $field) : ?>
				<div class="element">
					<div class="value field_<?php echo $field->name; ?>"><?php echo $field->display; ?></div>
				</div>
				<?php endforeach; ?>
			</div>
			<!-- EOF under-description-line2-nolabel block -->
			<?php endif; ?>
			

			<?php
				$readmore_forced = $this->params->get('show_readmore', 1) == -1 || $this->params->get('lead_strip_html', 1) == 1 ;
				$readmore_shown  = $this->params->get('show_readmore', 1) && ($uncut_length > $lead_cut_text || strlen(trim($item->fulltext)) >= 1);
				$readmore_shown  = $readmore_shown || $readmore_forced;
				$footer_shown = $readmore_shown || $item->event->afterDisplayContent;
				
			
			?>

			<?php if ( $footer_shown ) : ?>
			<footer class="group">
			<?php endif; ?>

			<?php if ( $readmore_shown ) : ?>
			<span class="readmore">				
				<a href="<?php echo $link_url; ?>" class="btn btn-xs" itemprop="url" <?php echo ($lead_link_to_popup ? 'onclick="var url = jQuery(this).attr(\'href\')+\''.$_tmpl_.'\'; fc_showDialog(url, \'fc_modal_popup_container\', 0, 0, 0, 0, {title: \'\'}); return false;"' : '');?> >
					<?php echo $item->params->get('readmore')  ?  $item->params->get('readmore') : JText::sprintf('FLEXI_READ_MORE', $item->title); ?> 
				</a>				
			</span>
			<?php endif; ?>

			
			<?php if ($item->event->afterDisplayContent) : ?>
			<!-- BOF afterDisplayContent -->
				<aside class="fc_afterDisplayContent group">
					<?php echo $item->event->afterDisplayContent; ?>
				</aside>
			<!-- EOF afterDisplayContent -->
			<?php endif; ?>
			
			
			<?php if ( $footer_shown ) : ?>
				</footer>
			<?php endif; ?>
			
			<?php // if ($this->params->get('show_title', 1)) : ?>
				</article>
			<?php // endif; ?>
			
		</div>
		<?php endfor; ?>
	</div>
<?php if ($this->params->get('lead_placement', 0)==1) : ?> </div> <?php endif; ?>
<?php endif; ?>

<?php
if ($this->limitstart != 0) $leadnum = 0;
if ($count > $leadnum) :
	//added to intercept more columns (see also css changes)
	$intro_cols                = $this->params->get('intro_cols', 1);
	$intro_cols_sm             = $this->params->get('intro_cols_sm', 1);
	$intro_cols_xs             = $this->params->get('intro_cols_xs', 1);
	$intro_cols_lg             = $this->params->get('intro_cols_lg', 1);
  	$intro_cols_classes        = array(1=>'one',2=>'two',3=>'three',4=>'four');
	$classnum                  = $intro_cols_classes[$intro_cols];
	$mansoryintrostyle         = $this->params->get('intro_mansory_style', 0);
	$intro_placement           = $this->params->get('intro_placement', 0);
	$gridclass_md 			   = $cols_gridclasses[$intro_cols];
	$gridclass_xs              = $cols_gridclasses_xs[$intro_cols_xs];
	$gridclass_sm   		   = $cols_gridclasses_sm[$intro_cols_sm];
	$gridclass_lg   		   = $cols_gridclasses_lg[$intro_cols];

	
	switch  ( $grid_framework  ) {

			case 0: $img_style    = '';   /* Flexicontent */
			        $img_class    = '';
					$img_position = '';
					break;
			case 1: $img_style    = 'img-';   /* bootstrap */
			        $img_class    = 'img-fluid ';
					$img_position = ' float-md-';
					break;			
			case 2: $img_style    = 'uk-border-';
					$img_class    = 'uk-thumbnail ';
					$img_position = ' uk-align-';   /*  Uikit */
					break;
	} ?>

<div class="<?php echo ( $grid_framework == 0 ) ? $classnum : ''; ?> <?php echo ($intro_placement == 1) ? 'mansoryintro' : '';?>">
<?php if ($intro_placement == 1) : ?>
		<div class="gutter-size "></div>
		<div class="grid-size <?php echo ( $grid_framework > 0 ) ? $gridclass_xs . ' ' . $gridclass_sm . ' ' . $gridclass_md . ' ' . $gridclass_lg : ' '; ?>"></div>
<?php endif; ?>

		<div class="introblock group <?php echo ( $grid_framework == 0 ) ? $classnum : ''; ?> <?php echo $mansoryintroclass; ?> <?php echo ($grid_framework==2) ? 'uk-grid' : '';?> <?php echo ( $grid_framework == 1 ) ? 'row' : ''; ?>">	
			<?php

			if ($intro_use_image ) {

			   switch  ( $intro_position  ) {
			    		case 0: $displayDataImg['img_position'] .= "left order-0 mr-md-3 mb-3 mt-md-3";
			    				break;
			    		case 1: $displayDataImg['img_position'] .= "right order-1 ml-md-3 mb-3 mt-md-3";
			    				break;			
			    		case 2: $displayDataImg['img_position'] .= "none mt-md-3";
			    				break;
			    }

				if ( $this->params->get('intro_image')) {
					$img_size_map   = array('l'=>'large', 'm'=>'medium', 's'=>'small', 'o'=>'original');
					$img_field_size = $img_size_map[ $this->params->get('intro_image_size' , 'l') ];
					$img_field_name = $this->params->get('intro_image');
						
				}


			}

			
			
			
			for ($i=$leadnum; $i<$count; $i++) :
				$item = $items[$i];
				$fc_item_classes = 'fc_bloglist_item';
				if ($doing_cat_order)
	     		$fc_item_classes .= ($i==0 || ($items[$i-1]->rel_catid != $items[$i]->rel_catid) ? ' fc_cat_item_1st' : '');
				/*$fc_item_classes .= ($i-$leadnum)%2 ? ' fceven' : ' fcodd';*/
				$fc_item_classes .= ' fccol'.($i%$intro_cols + 1);
				$gridclass    = "";
				$gridclass_sm = "";
				$gridclass_xs = "";

				if ( $grid_framework > 0 ) : 
					
					// grid 3 phone
					
					$gridclass_xs      .= $cols_gridclasses_xs[$intro_cols_xs] . ' ';
					
					// grid 3 cols tablet
					
					$gridclass_sm      .= $cols_gridclasses_sm[$intro_cols_sm] . ' ';
					
					$gridclass 		    = $cols_gridclasses[$intro_cols] . ' ';
					$gridclass 		   .= $cols_gridclasses_lg[$intro_cols];

					/* grid cols desktop*/
					if ( $intro_placement == 1 ) :
												
						switch  ( $mansoryintrostyle ) :
						
							case 0: 
									$gridclass  = $cols_gridclasses[$intro_cols] . ' ';
									$gridclass .= $cols_gridclasses_lg[$intro_cols];
									break;
							case 1: 
									$gridclass = ($i-$leadnum)%2 ? $cols_gridclasses_dbl[$intro_cols] . ' ' . $cols_gridclasses_dbl_lg[$intro_cols] : $cols_gridclasses[$intro_cols] . ' ' . $cols_gridclasses_lg[$intro_cols];
									break;
							case 2: 
									$gridclass = ($i-$leadnum)%2 ? $cols_gridclasses[$intro_cols] . ' ' . $cols_gridclasses_lg[$intro_cols] : $cols_gridclasses_dbl[$intro_cols] . ' ' . $cols_gridclasses_dbl_lg[$intro_cols];
									break;
							endswitch;
					endif;	
					$gridclass    .= ' ';
					

				endif; 

				$markup_tags = '<span class="fc_mublock">';
				foreach($item->css_markups as $grp => $css_markups) {
					if ( empty($css_markups) )  continue;
					$fc_item_classes .= ' fc'.implode(' fc', $css_markups);
					
					$ecss_markups  = $item->ecss_markups[$grp];
					$title_markups = $item->title_markups[$grp];
					foreach($css_markups as $mui => $css_markup) {
						$markup_tags .= '<span class="fc_markup mu' . $css_markups[$mui] . $ecss_markups[$mui] .'">' .$title_markups[$mui]. '</span>';
					}
				}
				$markup_tags .= '</span>';
				
				$custom_link = null;

				/* Introimage */


				if ($intro_use_image) :
					if (!empty($img_field_name) && ( array_key_exists($img_field_name, $item->fields)) )
					{
						// Render method 'display_NNNN_src' to avoid CSS/JS being added to the page
						FlexicontentFields::getFieldDisplay($item, $img_field_name, $values=null, $method='display_'.$img_field_size.'_src', 'category');
						
						$img_field = $item->fields[$img_field_name];
						$src = str_replace(JURI::root(), '', @ $img_field->thumbs_src[$img_field_size][0] );
						if ( $displayDataImg['intro_link_image_to'] && isset($img_field->value[0]) ) {
							$custom_link = ($v = unserialize($img_field->value[0])) !== false ? @ $v['link'] : @ $img_field->value[0]['link'];
						}
					} else {
						$src = flexicontent_html::extractimagesrc($item);
					}
					
					// Use default image form layout parameters
					if (!$src && isset($intro_type_default_imgs[$item->typealias]))  $src = $intro_type_default_imgs[$item->typealias];
					if (!$src && isset($intro_type_default_imgs['_OTHER_']))         $src = $intro_type_default_imgs['_OTHER_'];
					
					$RESIZE_FLAG = !$this->params->get('intro_image') || !$this->params->get('intro_image_size');
					if ( $src && $RESIZE_FLAG ) {
						// Resize image when src path is set and RESIZE_FLAG: (a) using image extracted from item main text OR (b) not using image field's already created thumbnails
						$w		= '&amp;w=' . $this->params->get('intro_width', 200);
						$h		= '&amp;h=' . $this->params->get('intro_height', 200);
						$aoe	= '&amp;aoe=1';
						$q		= '&amp;q=95';
						$zc		= $this->params->get('intro_method') ? '&amp;zc=' . $this->params->get('intro_method') : '';
						$ext = strtolower(pathinfo($src, PATHINFO_EXTENSION));
						$f = in_array( $ext, array('png', 'ico', 'gif') ) ? '&amp;f='.$ext : '';
						$conf	= $w . $h . $aoe . $q . $zc . $f;
						
						$base_url = (!preg_match("#^http|^https|^ftp|^/#i", $src)) ?  JURI::base(true).'/' : '';
						$displayDataImg['thumb'] = JURI::base(true).'/components/com_flexicontent/librairies/phpthumb/phpThumb.php?src='.$base_url.$src.$conf;
					} else {
						// Do not resize image when (a) image src path not set or (b) using image field's already created thumbnails
						$displayDataImg['thumb'] = $src;
					}
				endif;
				$link_url = $custom_link ? $custom_link : JRoute::_(FlexicontentHelperRoute::getItemRoute($item->slug, $item->categoryslug, 0, $item));
				$displayDataImg['link_url'] = $link_url;
				$displayDataImg['title_encoded'] = htmlspecialchars($item->title, ENT_COMPAT, 'UTF-8');
				
	
				if ($intro_link_to_popup) $_tmpl_ = (strstr($link_url, '?') ? '&' : '?'). 'tmpl=component';

				// MICRODATA document type (itemtype) for each item
				// -- NOTE: category's microdata itemtype is fallback if the microdata itemtype of the CONTENT TYPE / ITEM are not set
				$microdata_itemtype = $item->params->get( 'microdata_itemtype') ? $item->params->get( 'microdata_itemtype') : $microdata_itemtype_cat;
				$microdata_itemtype_code = $microdata_itemtype ? 'itemscope itemtype="http://schema.org/'.$microdata_itemtype.'"' : '';
			
			$background = ($i%2) ? 'background-primary' : 'background-secondary';

			?>
			
			<?php echo $intro_catblock ?
				'<div class="intro_catblock">'
					.($intro_catblock_title && @$globalcats[$item->rel_catid] ? $globalcats[$item->rel_catid]->title : '').
				'</div>' : ''; ?>
			
			<div id="fc_bloglist_item_<?php echo $i; ?>" class="<?php echo $fc_item_classes; ?> <?php echo ( $grid_framework > 0 ) ? $gridclass_xs . $gridclass_sm . $gridclass : ''; ?> " <?php echo $microdata_itemtype_code; ?> style="overflow: hidden;">
				
				<?php // if ($this->params->get('show_title', 1)) : ?>
					<article class="group grid-content <?php echo $background; ?>">
				<?php // endif; ?>
				
				
				<?php if ($item->event->beforeDisplayContent) : ?>
				<!-- BOF beforeDisplayContent -->
					<div class="fc_beforeDisplayContent group">
						<?php echo $item->event->beforeDisplayContent; ?>
					</div>
				<!-- EOF beforeDisplayContent -->
				<?php endif; ?>

				<?php if ($this->params->get('show_editbutton', 1)) : ?>
					<?php $editbutton = flexicontent_html::editbutton( $item, $this->params ); ?>
					<?php $statebutton = flexicontent_html::statebutton( $item, $this->params ); ?>
				<?php endif; ?>			
				<?php $deletebutton = flexicontent_html::deletebutton( $item, $this->params ); ?>
				<?php $approvalbutton = flexicontent_html::approvalbutton( $item, $this->params ); ?>

			
				<?php if ( $editbutton || $statebutton || $deletebutton || $approvalbutton ) : ?>
					<div class="adminbuttons d-flex">
						<?php if ($editbutton) : ?>
								<div class="fc_edit_link"><?php echo $editbutton;?></div>
						<?php endif; ?>

						<?php if ($statebutton) : ?>
								<div class="fc_state_toggle_link"><?php echo $statebutton;?></div>
						<?php endif; ?>
						<?php if ($deletebutton) : ?>
							<div class="fc_delete_link"><?php echo $deletebutton;?></div>
						<?php endif; ?>
						<?php if ($approvalbutton) : ?>
							<div class="fc_approval_request_link"><?php echo $approvalbutton;?></div>
						<?php endif; ?>
					</div>
				<?php endif; ?>

				
				<?php
					$header_shown =
						$this->params->get('show_comments_count', 1) ||
						$this->params->get('show_title', 1) || $item->event->afterDisplayTitle ||
						0; 
				?>
				
				<?php if ( $header_shown ) : ?>
				<header class="group">
				<?php endif; ?>

				<?php if ($this->params->get('show_comments_count')) : ?>
					<?php if ( isset($this->comments[ $item->id ]->total) ) : ?>
						<div <?php echo $_comments_container_params; ?> >
							<?php echo $this->comments[ $item->id ]->total; ?>
						</div>
					<?php endif; ?>
				<?php endif; ?>
					
				<?php if ($this->params->get('show_title', 1)) : ?>
					<h3 class="fc_item_title" itemprop="name">
						
						<?php if ($this->params->get('link_titles', 0)) : ?>
							<a href="<?php echo $link_url; ?>"><?php echo $item->title; ?></a>
						<?php else : ?>
							<?php echo $item->title; ?>
						<?php endif; ?>
					
					</h3>
				<?php endif; ?>
					
				
				<?php if ($item->event->afterDisplayTitle) : ?>
				<!-- BOF afterDisplayTitle -->
					<div class="fc_afterDisplayTitle group">
						<?php echo $item->event->afterDisplayTitle; ?>
					</div>
				<!-- EOF afterDisplayTitle -->
				<?php endif; ?>
				
				
				<?php echo $markup_tags; ?>
				
				<?php if ( $header_shown ) : ?>
				</header>
				<?php endif; ?>
					
				
				<?php if (isset($item->positions['above-description-line1'])) : ?>
				<!-- BOF above-description-line1 block -->
				<div class="lineinfo line1">
					<?php foreach ($item->positions['above-description-line1'] as $field) : ?>
					<div class="element">
						<?php if ($field->label) : ?>
						<span class="flexi label field_<?php echo $field->name; ?>"><?php echo $field->label; ?></span>
						<?php endif; ?>
						<div class="value field_<?php echo $field->name; ?>"><?php echo $field->display; ?></div>
					</div>
					<?php endforeach; ?>
				</div>
				<!-- EOF above-description-line1 block -->
				<?php endif; ?>
				

				
				<?php if (isset($item->positions['above-description-line1-nolabel'])) : ?>
				<!-- BOF above-description-nolabel-line1 block -->
				<div class="lineinfo line1">
					<?php foreach ($item->positions['above-description-line1-nolabel'] as $field) : ?>
					<div class="element">
						<div class="value field_<?php echo $field->name; ?>"><?php echo $field->display; ?></div>
					</div>
					<?php endforeach; ?>
				</div>
				<!-- EOF above-description-nolabel-line1 block -->
				<?php endif; ?>
				

				
				<?php if (isset($item->positions['above-description-line2'])) : ?>
				<!-- BOF above-description-line2 block -->
				<div class="lineinfo line2">
					<?php foreach ($item->positions['above-description-line2'] as $field) : ?>
					<div class="element">
						<?php if ($field->label) : ?>
						<span class="flexi label field_<?php echo $field->name; ?>"><?php echo $field->label; ?></span>
						<?php endif; ?>
						<div class="value field_<?php echo $field->name; ?>"><?php echo $field->display; ?></div>
					</div>
					<?php endforeach; ?>
				</div>
				<!-- EOF above-description-line2 block -->
				<?php endif; ?>
				

				
				<?php if (isset($item->positions['above-description-line2-nolabel'])) : ?>
				<!-- BOF above-description-nolabel-line2 block -->
				<div class="lineinfo line2">
					<?php foreach ($item->positions['above-description-line2-nolabel'] as $field) : ?>
					<div class="element">
						<div class="value field_<?php echo $field->name; ?>"><?php echo $field->display; ?></div>
					</div>
					<?php endforeach; ?>
				</div>
				<!-- EOF above-description-nolabel-line2 block -->
				<?php endif; ?>
				
				<div class="lineinfo image_descr <?php echo $intro_position == 2 ? 'flex-column' : 'flex-row';?>">

				<?php if ($intro_use_image && $src) : ?>
				<?php $displayDataImg['link_image']  = ($this->params->get('lead_link_image', 1)) ? true : false ?>							   
				<?php echo $blogimages->render($displayDataImg);?>
				<?php endif; ?>
				
				<?php if ($intro_use_description ) : ?>      
				<div class="intro-text mt-md-3">

				<?php if ( FlexicontentFields::getFieldDisplay($item, 'presse_kuerzel', $values=null, $method='display') ) { ?>	
				<?php  echo $item->fields['presse_kuerzel']->display;?>
				<?php } ?>
				<?php
					if ($this->params->get('intro_strip_html', 1)) :?>
						<p>
							<?php echo flexicontent_html::striptagsandcut( $item->fields['text']->display, $intro_cut_text, $uncut_length );?> 
						</p>
					<?php else :
						echo $item->fields['text']->display;
					endif;
				?>
				</div>
				<?php endif; ?>  
				
				</div>

				
				<?php if (isset($item->positions['under-description-line1'])) : ?>
				<!-- BOF under-description-line1 block -->
				<div class="lineinfo line3">
					<?php foreach ($item->positions['under-description-line1'] as $field) : ?>
					<div class="element">
						<?php if ($field->label) : ?>
						<span class="flexi label field_<?php echo $field->name; ?>"><?php echo $field->label; ?></span>
						<?php endif; ?>
						<div class="value field_<?php echo $field->name; ?>"><?php echo $field->display; ?></div>
					</div>
					<?php endforeach; ?>
				</div>
				<!-- EOF under-description-line1 block -->
				<?php endif; ?>
				

				
				<?php if (isset($item->positions['under-description-line1-nolabel'])) : ?>
				<!-- BOF under-description-line1-nolabel block -->
				<div class="lineinfo line3">
					<?php foreach ($item->positions['under-description-line1-nolabel'] as $field) : ?>
					<div class="element">
						<div class="value field_<?php echo $field->name; ?>"><?php echo $field->display; ?></div>
					</div>
					<?php endforeach; ?>
				</div>
				<!-- EOF under-description-line1-nolabel block -->
				<?php endif; ?>
				

				
				<?php if (isset($item->positions['under-description-line2'])) : ?>
				<!-- BOF under-description-line2 block -->
				<div class="lineinfo line4">
					<?php foreach ($item->positions['under-description-line2'] as $field) : ?>
					<div class="element">
						<?php if ($field->label) : ?>
						<span class="flexi label field_<?php echo $field->name; ?>"><?php echo $field->label; ?></span>
						<?php endif; ?>
						<div class="value field_<?php echo $field->name; ?>"><?php echo $field->display; ?></div>
					</div>
					<?php endforeach; ?>
				</div>
				<!-- EOF under-description-line2 block -->
				<?php endif; ?>
				

				
				<?php if (isset($item->positions['under-description-line2-nolabel'])) : ?>
				<!-- BOF under-description-line2-nolabel block -->
				<div class="lineinfo line4">
					<?php foreach ($item->positions['under-description-line2-nolabel'] as $field) : ?>
					<div class="element">
						<div class="value field_<?php echo $field->name; ?>"><?php echo $field->display; ?></div>
					</div>
					<?php endforeach; ?>
				</div>
				<!-- EOF under-description-line2-nolabel block -->
				<?php endif; ?>
				


				<?php
					$readmore_forced = $this->params->get('show_readmore', 1) == -1 || $this->params->get('intro_strip_html', 1) == 1 ;
					$readmore_shown  = $this->params->get('show_readmore', 1) && ($uncut_length > $intro_cut_text || strlen(trim($item->fulltext)) >= 1);
					$readmore_shown  = $readmore_shown || $readmore_forced;
					$footer_shown = $readmore_shown || $item->event->afterDisplayContent;
				
				?>

				<?php if ( $footer_shown ) : ?>
				<footer class="group">
				<?php endif; ?>

				<?php if ( $readmore_shown ) : ?>
				<span class="readmore">
					
					<a href="<?php echo $link_url; ?>" class="btn btn-xs" itemprop="url" <?php echo ($intro_link_to_popup ? 'onclick="var url = jQuery(this).attr(\'href\')+\''.$_tmpl_.'\'; fc_showDialog(url, \'fc_modal_popup_container\', 0, 0, 0, 0, {title: \'\'}); return false;"' : '');?> >
						<?php echo $item->params->get('readmore')  ?  $item->params->get('readmore') : JText::sprintf('FLEXI_READ_MORE', $item->title); ?> 
					</a>
					
				</span>
				<?php endif; ?>
					
				
				<?php if ($item->event->afterDisplayContent) : ?>
				<!-- BOF afterDisplayContent -->
					<div class="fc_afterDisplayContent group">
						<?php echo $item->event->afterDisplayContent; ?>
					</div>
				<!-- EOF afterDisplayContent -->
				<?php endif; ?>
				
				
				<?php if ( $footer_shown ) : ?>
					</footer>
				<?php endif; ?>
				
				<?php // if ($this->params->get('show_title', 1)) : ?>
					</article>
				<?php // endif; ?>	
			</div>
			<?php endfor; ?>
		</div>
	<?php if ($grid_framework==1) : ?> </div> <?php endif; ?>
	<?php endif; ?>
</div>
<div class="fcclear"></div>