							...
							<div class="card-header" id="heading_<?php echo $item->id;?>">
				    			<h3 class="wbc_item_title mb-0">
			    					<button class="btn btn-default btn-link btn-block text-left" data-toggle="collapse" data-target="#collapse_<?php echo $item->id;?>" aria-expanded="true" aria-controls="collapse_<?php echo $item->id;?>" type="button" >
			    						...
			    						<?php
			    						if ( $this->params->get('collapse_title',0) == 1 ) : ?>
			    						    <?php 
			    						    $collapse_fields = array(); 
			    						   
			    						    if ( $this->params->get('collapse_field') !== null && count($this->params->get('collapse_field')) > 0  )  : ?>

			    						    	<?php  $collapse_fields =  ( $this->params->get('collapse_field') ); ?>

			    						    	<?php foreach ($collapse_fields as $field) : 
			    						    	// alle felder von position collapse aufliste n 
												
												if ( !empty( $item->fields[$field]->display ) ) {
													$html_collapse .= '<span class="value value_'.  $field .'">'.  $item->fields[$field]->display . '</span>';
												}
												?>
														
								 		 		<?php endforeach; ?>
								 		 		
								 		 		<?php if ( empty($html_collapse) ) { 
								 		 			
														$html_collapse = $item->title;
								 		 		} ?>

			    						    <?php  endif; ?>		

										<?php else: $html_collapse = $item->title; ?>	
											
										<?php endif; ?>

										<?php echo $html_collapse; ?>
										<?php $html_collapse = ''; ?>
