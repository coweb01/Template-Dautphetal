...
<ul class="nav akdropdown <?php echo $class_sfx;?>"<?php
	$tag = '';
...
foreach ($list as $i => &$item) :
	...
	switch ($item->level) :
		...
		case 2:  
 				 if (  $menuecol == 1 ) :
					  ...
					  echo '<div id="nav-col-' .$col. '-'. $item->id . '-' . $module->id .'" class="nav-col nav-col-'. $col.'" ' . $stylecolumn . '>'. "\n";
					  echo '<ul>'. "\n";
					  echo '<li'.$class.'>'. "\n";
					  $col++;
					  $open_cols = true;
		...
		case 1:  
				 echo '<li'.$class.'>';
				 echo (isset( $toggleLink)) ? $toggleLink : '';
	...
	switch ($item->type) :
		...
		case 'component':
		case 'heading':
			require JModuleHelper::getLayoutPath('mod_menu', 'offcanvsbts4_'.$item->type);
			break;

		default:
			require JModuleHelper::getLayoutPath('mod_menu', 'offcanvsbts4_url');
			break;
	endswitch;
